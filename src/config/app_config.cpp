...
pt::ptree VideoStreamCfgToJson::operator()(
		const FileVideoStreamCfg & cfg) const {
	pt::ptree val;
...
const Cfgs::VideoStreamCfg VideoStreamCfgJsonify::fromJson(
		const pt::ptree &root) const {
	boost::optional<const pt::ptree&> value = root.get_child_optional(
			"FileVideoStreamCfg");
	if (value) {
		pt::ptree node = value.get();
		FileVideoStreamCfg cfg;
		std::string filename = node.get<std::string>("filename",
				cfg.filename());
...
PVideoStream VideoStreamFactory::operator()(
		const FileVideoStreamCfg & cfg) const {
...
// SceneDetectorCfg
//-----------------------------------------------------------------------------
pt::ptree SceneDetectorCfgToJson::operator()(
		const NaiveSceneDetectorCfg & cfg) const {
	...
	params_node.put("maxFramesToLooseTrack",
...
pt::ptree SceneDetectorCfgToJson::operator()(
		const ShowStreamDetectorCfg & cfg) const {
	pt::ptree node;
	pt::ptree val;
...
pt::ptree SceneDetectorCfgToJson::operator()(
		const VFileSceneDetectorCfg & cfg) const {
	pt::ptree node;
	pt::ptree val;
	val.put("faceLandmarksModelFilename", cfg.faceLandmarksModelFilename());
...
const Cfgs::SceneDetectorCfg SceneDetectorCfgJsonify::fromJson(
		const pt::ptree &root) const {
	boost::optional<const pt::ptree&> value = root.get_child_optional(
			"NaiveSceneDetectorCfg");
	if (value) {
		boost::optional<const pt::ptree&> ps_node =
				value.get().get_child_optional("NaiveSceneDetectorParameters");
		NaiveSceneDetectorCfg cfg;
		if (ps_node) {
			const pt::ptree& node = ps_node.get();
			cfg.params().minFaceSize(
					node.get<int>("minFaceSize", cfg.params().minFaceSize()));
			cfg.params().maxFaceSize(
					node.get<int>("maxFaceSize", cfg.params().maxFaceSize()));
			cfg.params().centroidThresh(
					node.get<double>("centroidThresh",
							cfg.params().centroidThresh()));
			cfg.params().areaThresh(
					node.get<double>("areaThresh", cfg.params().areaThresh()));
			cfg.params().maxFramesToLooseTrack(
					node.get<int>("maxFramesToLooseTrack",
							cfg.params().maxFramesToLooseTrack()));
			cfg.params().maxFaces(
					node.get<int>("maxFaces", cfg.params().maxFaces()));
		}
		cfg.visualDebugging(
				value.get().get<bool>("visualDebugging",
						cfg.visualDebugging()));
	...
	if (value) {
		...
		cfg.faceLandmarksModelFilename(
				value.get().get<std::string>("faceLandmarksModelFilename",
						cfg.faceLandmarksModelFilename()));
		return Cfgs::SceneDetectorCfg(cfg);
	}
	...
	if (value) {
		...
		cfg.faceLandmarksModelFilename(
				value.get().get<std::string>("faceLandmarksModelFilename",
						cfg.faceLandmarksModelFilename()));
		cfg.outFile(value.get().get<std::string>("outFile", cfg.outFile()));
...
PSceneDetector SceneDetectorFactory::operator()(
		const NaiveSceneDetectorCfg & cfg) {
	NaiveSceneDetector::PDebugger dbg(
			cfg.visualDebugging() ?
					new NaiveSceneDetectorDebugger() :
					new NaiveSceneDetector::Debugger());
	return PSceneDetector(
			new NaiveSceneDetector(std::move(_vs), std::move(_sdl),
					std::move(dbg), cfg.params()));
}

PSceneDetector SceneDetectorFactory::operator()(
		const ShowStreamDetectorCfg & cfg) {
	return PSceneDetector(new ShowStreamDetector(std::move(_vs), cfg));
}

PSceneDetector SceneDetectorFactory::operator()(
		const VFileSceneDetectorCfg & cfg) {
	FileVideoStream *fvs = dynamic_cast<FileVideoStream *>(_vs.release());
	if (fvs == nullptr) {
		throw std::invalid_argument(
				"VFileSceneDetector requires a FileVideoStream.");
	}
...
//-----------------------------------------------------------------------------
pt::ptree SceneDetectorListenerCfgToJson::operator()(
		const SceneNilListenerCfg & cfg) const {
	pt::ptree node;
	pt::ptree val;
...
pt::ptree SceneDetectorListenerCfgToJson::operator()(
		const FileSceneDetectorListenerCfg & cfg) const {
	pt::ptree val;
	val.put("filename", cfg.filename());
	pt::ptree node;
...
pt::ptree SceneDetectorListenerCfgToJson::operator()(const FPCPSceneDetectorListenerCfg & cfg) const {
	pt::ptree node;

	pt::ptree val;
	val.put("fp_id", cfg.fp_id);
	val.put("url", cfg.url);
	val.put("get_timeout", cfg.get_timeout);
	node.put_child("FPCPSceneDetectorListenerCfg",	val);
	return node;
}

const Cfgs::SceneDetectorListenerCfg SceneDetectorListenerCfgJsonify::fromJson(
		const pt::ptree &root) const {
	boost::optional<const pt::ptree&> value = root.get_child_optional(
			"FileSceneDetectorListenerCfg");
	if (value) {
...
PSceneDetectorListener SceneDetectorListenerFactory::operator()(
		const SceneNilListenerCfg & cfg) const {
...
PSceneDetectorListener SceneDetectorListenerFactory::operator()(
		const FPCPSceneDetectorListenerCfg & cfg) const {
	return PSceneDetectorListener(new FPCPSceneDetectorListener(cfg));
}
...
void AppConfig::toJson(pt::ptree *root) const {
	pt::ptree vscNode = _videoStreamCfg.apply_visitor(
			_videoStreamCfgJsonify.toJson());
	root->put_child("VideoSourceCfg", vscNode);
	pt::ptree sdNode = _sceneDetectorCfg.apply_visitor(
			_sceneDetectorCfgJsonify.toJson());
	root->put_child("SceneDetectorCfg", sdNode);
	pt::ptree sdlNode = _sceneDetectorListenerCfg.apply_visitor(
			_sceneDetectorListenerCfgJsonify.toJson());
...
void AppConfig::fromJson(std::istream &in) {
	...
	boost::optional<pt::ptree&> value = root.get_child_optional(
			"VideoSourceCfg");
	if (value) {
	...
	if (value) {
		_sceneDetectorListenerCfg = _sceneDetectorListenerCfgJsonify.fromJson(
				value.get());
	} else {
		_sceneDetectorListenerCfg = FileSceneDetectorListenerCfg();
	}
...
PSceneDetector AppConfig::createSceneDetector() const {
	...
	PSceneDetectorListener sdl = _sceneDetectorListenerCfg.apply_visitor(
			_sceneDetectorListenerFactory);
	SceneDetectorFactory factory = SceneDetectorFactory(std::move(vs),
			std::move(sdl));
...
void DefaultCfgs::create(const std::string &filenamePrefix) {
	...
	std::string vfile_scene_detector = filenamePrefix
			+ "_vfile_scene_detector.json";
