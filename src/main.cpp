...
int main(int argc, char** argv) {
	...
	("dst_display", po::value<bool>()->default_value(false),
			"Don't try to detect, but snow camera video-stream on the screen.") //
	("scn_no_visual", po::value<bool>()->default_value(false),
			"Do not visualize scene detection (no video scene on the screen).") //
	...
	("cam_height", po::value<int>()->default_value(-1),
			"Height of camera's frame") //
	("cam_fps", po::value<int>()->default_value(-1), "Camera's FPS") //
	("cam_brightness", po::value<int>()->default_value(-1),
			"Camera's brightness (int): min=0 max=255 step=1 default=128") //
	("cam_contrast", po::value<int>()->default_value(-1),
			"Camera's contrast (int): min=0 max=255 step=1 default=128") //
	("cam_saturation", po::value<int>()->default_value(-1),
			"Camera's saturation (int): min=0 max=255 step=1 default=128") //
	("cam_gain", po::value<int>()->default_value(-1),
			"Camera's gain (int): min=0 max=255 step=1 default=0") //
	("cam_exposure_auto", po::value<int>()->default_value(-1),
			"Camera's exposure_auto (menu): min=0 max=3 default=3 value=3") //
	("cam_exposure_absolute", po::value<int>()->default_value(-1),
			"Camera's exposure_absolute (int): min=3 max=2047 step=1 default=250") //
	("cam_autofocus", po::value<bool>()->default_value(true),
			"Camera's focus_auto (bool): default=1") //
	("cam_fourcc", po::value<std::string>()->default_value(""),
			"Camera's fourcc, for instance MJPG, YUYV, H264")
	//TODO:  These are not worked ...
	("cam_sharpness", po::value<int>()->default_value(-1),
			"Camera's sharpness (int): min=0 max=255 step=1 default=128") //
	("cam_backlight_compensation", po::value<int>()->default_value(-1),
			"Camera's backlight_compensation (int): min=0 max=1 step=1 default=0") //
	("cam_white_balance_temperature", po::value<int>()->default_value(-1),
			"Camera's white_balance_temperature (int): min=2000 max=6500 step=1 default=4000");
	/*
	...
	PVideoStream src;
	std::string src_file =
			vm.count("src_file") ? vm["src_file"].as<std::string>() : "";
	if (src_file.length()) {
	...
	std::string res_path =
			vm.count("res_path") ? vm["res_path"].as<std::string>() : "./";
	LOG_INFO("Will load resource files from " << res_path);
	std::shared_ptr<DnnFaceRecognitionNet> rn(
			new DnnFaceRecognitionNet(
					res_path + "dlib_face_recognition_resnet_model_v1.dat",
	...
	fpcpClntParams.access_key =
			vm.count("access_key") ? vm["access_key"].as<std::string>() : "";
	if (fpcpClntParams.access_key.size() == 0) {
		LOG_WARN("No access key.");
	} else {
		LOG_INFO("access key is " << fpcpClntParams.access_key);
	}

	fpcpClntParams.secret_key =
			vm.count("secret_key") ? vm["secret_key"].as<std::string>() : "";
	if (fpcpClntParams.secret_key.size() == 0) {
	...
	if (vm["dst_display"].as<bool>()) {
		...
		SceneDetector* sd = new SceneDetector(
				PSceneDetectorListener(
						new FpcpConnector(fpcpClntParams)),
				rm);
