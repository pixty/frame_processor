...
inline long x_center(dlib::rectangle &r) {
	return r.left() + (r.right() - r.left())/2;
}

inline long y_center(dlib::rectangle &r) {
	return r.top() + (r.bottom() - r.top())/2;
}

void adjust_rect(dlib::rectangle &old_rect, dlib::rectangle &new_rect) {
	int dx = x_center(new_rect) - x_center(old_rect);
	int dy = y_center(new_rect) - y_center(new_rect);
...
//	dy = dy < 0 ? -1 : dy;
	...
	old_rect.set_left(old_rect.left() + dx);
	old_rect.set_right(old_rect.right() + dx);
	old_rect.set_top(old_rect.top() + dy);
	old_rect.set_bottom(old_rect.bottom() + dy);
}

void FaceDetector::demo1(cv::Mat &out, const int frame,
		std::vector<dlib::rectangle> faces) {
	static std::vector<dlib::rectangle> init_faces = faces;
	const double fps = 30; // 1 second

	static int startFrom = 0;
	static bool rectOn = false;
	static int state = 0;
	static int expectedNumFaces;

	for (int i = 0; i < faces.size() && i < init_faces.size(); i++) {
		adjust_rect(init_faces[i], faces[i]);
	}
	...
	// 2 - Solid + ...
	// 3 - Solid + name
	static int stateLen = 2 * fps; // 2 second length
	...
	case 0:
		if ((stateLen >> 2)&1) {
			effect(out, init_faces, 0);
		}
		break;
	case 1:
		effect(out, init_faces, 3);
		break;
	case 2:
		effect(out, init_faces, 4 + ((stateLen >> 3)&3));
		break;
	case 3:
		effect(out, init_faces, 8);
		break;
	}
}
...
void FaceDetector::effect(cv::Mat&out,
	...
	std::string *persons = faces.size() == 2 ? d_s : stas;

	static std::string suffix(" Pixty founder");
	for (unsigned long i = 0; i < faces.size(); ++i) {
		...
		int x;
		if (faces.size() == 2) {
			x = out.size().width / 2.0 - txt.length() * 16.0 * (1 - i)
					+ (i * 32.0);
		} else {
		...
		case 7:
			// border and ...
			rounded_rectangle(out, cv_f, cv::Scalar(255, 255, 252), 1, 16, 5);
			put_text(out, wait_str[effect-4], x, y);
			break;
		case 8:
			// border and name
			rounded_rectangle(out, cv_f, cv::Scalar(58, 242, 252), 1, 16, 5);
